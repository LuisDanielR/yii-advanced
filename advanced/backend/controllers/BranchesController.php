...
class BranchesController extends Controller
{
    ...
    {
        $comments = Yii::$app->db2->createCommand('SELECT * from product')->queryAll();
        print_r($comments);
        die();
    ...
    public function actionImportExcel()
    {
        ...
        for($row = 1; $row <= $highestRow; $row++){
            ...
            if(!empty($rowData[0][0])){
                $data[] = [$rowData[0][0],$rowData[0][1],$rowData[0][2],
                    $rowData[0][3],date('Y-m-d H:i:s'),$rowData[0][5]];
            }
